...
printf "%s\n" "workspaces%{B$invert_bg}%{F$invert_fg}${output}%{F-}%{B-}"
}
...
justtext(){
  printf "%s\n" "justtext%{B$invert_bg}%{F$invert_fg}JustText%{F-}%{B-}"
}
...
while :; do workspaces; sleep 5.1s; done > "$fifo" &
  ...
  while :; do clock; sleep 1s; done > "$fifo" &
    ...
    while :; do justtext; sleep 3s; done > "$fifo" &
      #################
      # Build the bar #
      #################
      ...
      while read -r line ; do
        ...
        printf "%s\n" "%{l}${workspaces}%{c}${justtext}%{r}${clock}"
      done < "$fifo" | lemonbar \
        -g "$panel_dimensions" \
